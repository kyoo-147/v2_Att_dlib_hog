# Face Recognition Atthendance - v2 Update Version
# First GUI / Face Register New User
import dlib
import numpy as np
import cv2
import os
...
class Face_Register:
    def __init__(self):
        ...
        # Tkinter GUI
        self.win = tk.Tk()
        self.win.title("X_Register - @minhcuong-AILab")

        # PLease modify window size here if needed
        self.win.geometry("1300x800")

        # Save background
        self.background_image = PhotoImage(file="uii/main1.png")  # Đường dẫn đến tệp ảnh nền
        ...
        self.label.pack(side=tk.LEFT, padx=105)
        self.frame_left_camera.pack()

        # GUI right part
        #self.frame_right_info.create_rectangle(fill = '#1F2022')
        self.frame_right_info = tk.Frame(self.win,background='#07080A', highlightthickness=2, highlightbackground='white')
        #self.frame_right_info - tk.Label(self.frame_right_info,background='black')
        self.label_cnt_face_in_database = tk.Label(self.frame_right_info, text=str(self.existing_faces_cnt),fg='#D7A7A6',background='#07080A')
        self.label_cnt_face_in_database.grid(row=0, column=0)
        self.label_fps_info = tk.Label(self.frame_right_info, text="",fg='#D7A7A6',background='#07080A')
        ...
        self.label_warning = tk.Label(self.frame_right_info,fg='#D7A7A6',background='#07080A')
        self.label_face_cnt = tk.Label(self.frame_right_info, text="Số khuôn mặt trên khung hình hiện tại: ",fg='#D7A7A6',background='#07080A')
        self.log_all = tk.Label(self.frame_right_info,fg='#D7A7A6',background='#07080A')
    ...
    def GUI_clear_data(self):
        ...
        self.log_all["text"] = "Hình ảnh khuôn mặt và `features_all.csv` đã xóa!"
    ...
    def GUI_info(self):
        tk.Label(self.frame_right_info,
                 text="Đăng kí thông tin \n Hệ thống điểm danh AI-Lab",fg="#D7A7A6",background='#07080A',
                 font=self.font_title).grid(row=0, column=0, columnspan=3, sticky=tk.W, padx=2, pady=20)

        tk.Label(self.frame_right_info,
                 text="FPS: ",fg='#D7A7A6',background='#07080A').grid(row=1, column=0, columnspan=2, sticky=tk.W, padx=5, pady=2)
        self.label_fps_info.grid(row=1, column=2, sticky=tk.W, padx=5, pady=2)

        tk.Label(self.frame_right_info,
                 text="Khuôn mặt trong dữ liệu lưu trữ: ",background='#07080A',fg='#D7A7A6').grid(row=2, column=0, columnspan=2, sticky=tk.W, padx=5, pady=2)
        self.label_cnt_face_in_database.grid(row=2, column=2, columnspan=3, sticky=tk.W, padx=5, pady=2)

        tk.Label(self.frame_right_info,
                 text="Khuôn mặt trong khung hình hiện tại: ",fg='#D7A7A6',background='#07080A').grid(row=3, column=0, columnspan=2, sticky=tk.W, padx=5, pady=2)
        ...
        tk.Label(self.frame_right_info,
                 ...
                 text=" Xóa ảnh khuôn mặt",fg='#D7A7A6',background='#07080A').grid(row=5, column=0, columnspan=2, sticky=tk.W, padx=5)#, pady=20)
        tk.Button(self.frame_right_info,
                  text='Xóa',fg='#D7A7A6',
                  command=self.GUI_clear_data,background='#07080A').grid(row=6, column=0, columnspan=3, sticky=tk.W, padx=5)#, pady=2)

        # Step 2: Input name and create folders for face
        tk.Label(self.frame_right_info,
                 font=self.font_step_title,
                 text=" Nhập tên người dùng",fg='#D7A7A6',background='#07080A').grid(row=7, column=0, columnspan=2, sticky=tk.W, padx=5)#, pady=20)

        tk.Label(self.frame_right_info, text="Tên: ",fg='#D7A7A6',background='#07080A').grid(row=8, column=0, sticky=tk.W, padx=5, pady=0)
        self.input_name.grid(row=8, column=1, sticky=tk.W, padx=0, pady=2)

        tk.Button(self.frame_right_info,
                  text='Nhập vào',fg='#D7A7A6',
                  command=self.GUI_get_input_name,background='#07080A').grid(row=8, column=2, padx=5)

        # Step 3: Save current face in frame
        tk.Label(self.frame_right_info,
                 font=self.font_step_title,
                 text=" Lưu hình ảnh khuôn mặt",fg='#D7A7A6',background='#07080A').grid(row=9, column=0, columnspan=2, sticky=tk.W, padx=5)#, pady=20)

        tk.Button(self.frame_right_info,
                  text='Lưu khuôn mặt hiện tại',
                  command=self.save_current_face,fg='#D7A7A6',background='#07080A').grid(row=10, column=0, columnspan=3, sticky=tk.W)
    ...
    def check_existing_faces_cnt(self):
        ...
        # Nếu không được lưu trữ, chương trình sẽ bắt đầu lưu trữ với người dùng đầu tiên / Start from person_1
        else:
            self.existing_faces_cnt = 0
    ...
    def create_face_folder(self):
        ...
        self.log_all["text"] = "Đã tạo dữ liệu tại \n" + self.current_face_dir 
        logging.info("\n%-40s %s", "Tạo dữ liệu tại:", self.current_face_dir)
        ...
        if self.face_folder_created_flag:
            if self.current_frame_faces_cnt == 1:
                if not self.out_of_range_flag:
                    ...
                    self.log_all["text"] = "Đã lưu dữ liệu tại \n" + self.current_face_dir + "/img_face_" + str(
                        self.ss_cnt) + ".jpg\"" 
                    self.face_ROI_image = cv2.cvtColor(self.face_ROI_image, cv2.COLOR_BGR2RGB)

                    cv2.imwrite(self.current_face_dir + "/img_face_" + str(self.ss_cnt) + ".jpg", self.face_ROI_image)
                    logging.info("%-40s %s/img_face_%s.jpg", "Đã lưu dữ liệu tại： ",
                                 str(self.current_face_dir), str(self.ss_cnt) + ".jpg")
                else:
                    self.log_all["text"] = "Thông báo: Xin vui lòng không ra khỏi phạm vi nhận diện!"
            else:
                self.log_all["text"] = "Cảnh báo: Không nhận thấy khuôn mặt trong khung hình hiện tại!"
        else:
            self.log_all["text"] = "Thông báo: Vui lòng qua bước 2!"
    ...
    def get_frame(self):
        try:
            ...
            print("Lỗi: Không có video đầu vào!!!")
            ...
            if len(faces) != 0:
                ...
                for k, d in enumerate(faces):
                    ...
                    if (d.right() + self.ww) > 640 or (d.bottom() + self.hh > 480) or (d.left() - self.ww < 0) or (
                            d.top() - self.hh < 0):
                        self.label_warning["text"] = "NGOÀI PHẠM VI NHẬN DIỆN"
                        self.label_warning['fg'] = 'red'
                        self.out_of_range_flag = True
                        color_rectangle = (255, 0, 0)
                    else:
                        self.out_of_range_flag = False
                        self.label_warning["text"] = "TRONG PHẠM VI CÓ THỂ NHẬN DIỆN"
                        self.label_warning['fg'] = 'red'
